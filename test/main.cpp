...
int main()
{
  "radians to degree (double)"_test = [](auto && args) {
    const auto & [radians, degree] = args;
    constexpr auto epsilon = 2 * std::numeric_limits<double>::epsilon();
    expect(geo::radians_to_degrees(radians) - degree < epsilon);
  ...
  "radians to degree (float)"_test = [](auto && args) {
    const auto & [radians, degree] = args;
  ...
  "degree to radians (double)"_test = [](const auto & args) {
    ...
    constexpr auto epsilon = 2 * std::numeric_limits<double>::epsilon();
    expect(geo::degrees_to_radians(degree) - radians < epsilon);
  } | std::vector<std::pair<double, double>>{
    {0.0, 0.0},
